...
class HutangBahanHabisPakaiController extends Controller
{
    public function index()
    {
        return view('kasir.hutang-bahan-habis-pakai.hutang-bahan-habis-pakai');
    }

    public function getDataHutangBahanHabisPakai()
    {
        $dataHutang = HutangBahanHabisPakai::with([
            'supplier',
    ...
    public function getDataDetailHutangBahanHabisPakai($noFaktur)
    {
        $detailHutang = HutangBahanHabisPakai::with([
            'supplier',
            'metodePembayaran',
            ...
            'restockBahanHabisPakai',
            'restockBahanHabisPakai.supplier',
            'restockBahanHabisPakai.depot',
            'restockBahanHabisPakai.restockBahanHabisPakaiDetail.bahanHabisPakai',
            'restockBahanHabisPakai.restockBahanHabisPakaiDetail.batchBahanHabisPakai',
        ...
        $restock = $detailHutang->restockBahanHabisPakai;

        return response()->json([
            'message' => 'Detail data hutang berhasil diambil.',
            'data' => [
                'id' => $detailHutang->id,
                'no_faktur' => $detailHutang->no_faktur,
                'status_hutang' => $detailHutang->status_hutang,
                'tanggal_hutang' => $detailHutang->tanggal_hutang,
                'tanggal_jatuh_tempo' => $detailHutang->tanggal_jatuh_tempo,
                'tanggal_pelunasan' => $detailHutang->tanggal_pelunasan,
                'total_hutang' => $detailHutang->total_hutang,
                'created_at' => $detailHutang->created_at,
                'updated_at' => $detailHutang->updated_at,

                'supplier' => $detailHutang->supplier,
                'metode_pembayaran' => $detailHutang->metodePembayaran,
                'dibuat_oleh' => $detailHutang->dibuatOleh,
                'diupdate_oleh' => $detailHutang->diupdateOleh,

                'restock_bahan_habis_pakai' => $restock ? [
                    'id' => $restock->id,
                    'no_faktur' => $restock->no_faktur,
                    'tanggal_terima' => $restock->tanggal_terima,
                    'tanggal_jatuh_tempo' => $restock->tanggal_jatuh_tempo,
                    'status_restock' => $restock->status_restock,
                    'total_tagihan' => $restock->total_tagihan,
                    'depot' => $restock->depot,
                    'supplier' => $restock->supplier,
                    'restock_bahan_habis_pakai_detail' => $restock->restockBahanHabisPakaiDetail->map(function ($item) {
                        return [
                            'id' => $item->id,
                            'qty' => $item->qty,
                            ...
                            'bahan_habis_pakai' => $item->bahanHabisPakai,
                            'batch_bahan_habis_pakai' => $item->batchBahanHabisPakai,
                        ];
                    }),
                ] : null,
            ],
    ...
    public function halamanPembayaranHutangBahanHabisPakai($noFaktur)
    {
        $dataHutang = HutangBahanHabisPakai::with([
            'supplier',
            'metodePembayaran',
        ...
        if (!$dataHutang) {
            return redirect()->route('kasir.hutang.bahan.habis.pakai')
                ->with('error', 'Data hutang tidak ditemukan.');
        }

        return view('kasir.hutang-bahan-habis-pakai.pembayaran-hutang-bahan-habis-pakai', compact('dataHutang', 'dataMetodePembayaran'));
    }
    ...
    public function transaksiCash(Request $request, $noFaktur)
    {
        ...
        $hutang = HutangBahanHabisPakai::where('no_faktur', $noFaktur)->first();
        ...
        if (!$hutang->restockBahanHabisPakai) {
            return response()->json([
                'success' => false,
        ...
        if ($hutang->restockBahanHabisPakai->status_restock !== 'Succeed') {
            return response()->json([
                'success' => false,
        ...
        return response()->json([
            'success'   => true,
            'message'   => 'Pembayaran hutang Bahan Habis Pakai dengan metode cash berhasil diproses.',
    ...
    public function transaksiTransfer(Request $request, $noFaktur)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Pembayaran hutang Bahan Habis Pakai dengan metode transfer berhasil diproses.',
                'data'    => $hutang->fresh(['supplier', 'metodePembayaran', 'restockBahanHabisPakai']),
            ]);
        } catch (\Throwable $th) {
            DB::rollBack();
    ...
    public function getDataRiwayatHutangBahanHabisPakai()
    {
        $dataRiwayatHutang = HutangBahanHabisPakai::with([
            'supplier',
            'restockBahanHabisPakai.depot',
        ])
        ...
        return DataTables::of($dataRiwayatHutang)
                ...
                if ($search) {
                    $query->where(function ($q) use ($search) {
                        $q->where('no_faktur', 'like', "%{$search}%")
                            ...
                            ->orWhereHas('restockBahanHabisPakai.depot', function ($depot) use ($search) {
            ...
            ->addColumn('action', function ($row) {
                ...
                <button 
                    type="button"
                    class="button-detail-riwayat-hutang-bhp px-4 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600"
                    data-no-faktur="' . e($row->no_faktur) . '">
                    Detail
                </button>
            ';
    ...
    public function getDataDetailRiwayatHutangBahanHabisPakai($noFaktur)
    {
        $hutang = HutangBahanHabisPakai::with([
            'supplier',
            'metodePembayaran',
        ...
        $restock = $hutang->restockBahanHabisPakai;
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'restock' => [
                    ...
                    'detail_item' => collect($restock?->restockBahanHabisPakaiDetail ?? [])->map(function ($detail, $index) {
                        ...
                        return [
                            'no' => $index + 1,
                            'kode_bhp' => $detail->bahanHabisPakai?->kode ?? '-',
                            'nama_bhp' => $detail->bahanHabisPakai?->nama_barang ?? '-',
                            'qty' => $detail->qty ?? 0,
