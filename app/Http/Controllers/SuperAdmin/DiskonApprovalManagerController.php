...
class DiskonApprovalManagerController extends Controller
{
    ...
    {
        foreach (['id', 'detail_id', 'pembayaran_detail_id', 'pembayaranDetailId'] as $key) {
    ...
    private function extractPersen(array $item, $detail = null): float
    {
        ...
        $subtotal = (float) data_get($detail, 'subtotal', 0);
    ...
    private function applySearch($query): void
    {
        ...
        $query->where(function ($q) use ($search) {
            $q->where('reason', 'like', "%{$search}%")
                ...
                ->orWhereHas('pembayaran.emr.kunjungan.pasien', function ($p) use ($search) {
                ...
                ->orWhereHas('approver', function ($a) use ($search) {
    ...
    private function buildDiskonData($approval): array
    {
        ...
        $details = collect(data_get($approval, 'pembayaran.pembayaranDetail', []));
        ...
        // fallback: kalau JSON diskon_items kosong / tidak cocok, ambil dari pembayaran_detail
        ...
        foreach ($selectedMap as $detailId => $persen) {
            ...
            $qty = (float) data_get($detail, 'qty', 1);
            $harga = (float) data_get($detail, 'harga', 0);
            $subtotal = (float) data_get($detail, 'subtotal', 0);
            ...
            $namaItem = $this->resolveNamaItem($detail);
            $jenis = $this->resolveJenis($detail);
    ...
    public function getDataBelumApprove()
    {
        ...
        return DataTables::eloquent($query)
            ...
            ->addColumn('diskon_items_detail', function ($row) {
                ...
                $detailUrl = route('super.admin.diskon.detail_items', $row->id);
    ...
    public function getDataSudahApprove()
    {
        ...
        return DataTables::eloquent($query)
            ...
            ->addColumn('diskon_items_detail', function ($row) {
                ...
                $detailUrl = route('super.admin.diskon.detail_items.sudah.approve', $row->id);
    ...
    public function getDetailSudahApprove(DiskonApproval $approval)
    {
        $this->ensureManager();
