<section class="space-y-5">
       ...
       bg-gradient-to-br from-sky-500 to-cyan-500 text-white shadow-md shrink-0">
            ...
            <div class="min-w-0">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                    Kelola data pemesanan restock Bahan Habis Pakai dari supplier secara teratur dan terstruktur.
                    <span class="hidden sm:inline">
                        Gunakan pencarian untuk menemukan data restock Bahan Habis Pakai dengan lebih cepat.
                    </span>
                </p>
            </div>
        </div>
        ...
        <div class="flex justify-center md:justify-end">
            <button id="button-open-modal-create-restock-bhp" type="button"
    ...
    <div
        ...
        <div class="px-3 sm:px-4 md:px-6 py-3 border-b border-slate-200 dark:border-slate-700">
            <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                ...
                <div class="flex items-center gap-2 text-sm w-full md:w-auto">
                    ...
                    <select id="restock-bhp-page-length"
                ...
                <div class="w-full md:w-auto">
                    <div class="relative">
                        ...
                        <input type="text" id="restock-bhp-search-input"
        ...
        <div class="overflow-x-auto">
            <table id="table-restock-bahan-habis-pakai"
        ...
        <div
            ...
            <div id="restock-bhp-custom-info" class="text-xs md:text-sm text-slate-600 dark:text-slate-300"></div>

            <div class="w-full md:w-auto overflow-x-auto">
                <ul id="restock-bhp-custom-pagination"
...
<div id="modal-create-restock-bhp"
    ...
    <div class="w-full max-w-7xl max-h-[95vh] overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="text-xl font-bold text-slate-800">
                        Form Pemesanan Restock Bahan Habis Pakai
                    </h3>
                    <p class="mt-1 text-sm text-slate-500">
                        Lengkapi data transaksi restock dan detail item Bahan Habis Pakai.
                    </p>
                </div>
            </div>

            <button type="button" id="button-close-modal-create-restock-bhp"
        ...
        <div class="max-h-[calc(95vh-88px)] overflow-y-auto">
            <form id="form-create-restock-bhp" action="#" method="POST" class="p-6 space-y-6">
                @csrf
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-2 my-4">
                        <div>
                            ...
                            <input type="text" name="no_faktur"
                                class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm focus:border-emerald-500 focus:outline-none focus:ring-4 focus:ring-emerald-100"
                                placeholder="Contoh: INV-RST-BHP-0001">
                        </div>
                    ...
                    <div id="restock-detail-container" class="space-y-4">
                        ...
                        <div class="detail-row rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <div class="grid grid-cols-1 gap-4 xl:grid-cols-4">
                                <div class="xl:col-span-2">
                                    <label class="mb-1.5 block text-sm font-medium text-slate-700">Bahan Habis
                                        Pakai</label>
                                    <select name="details[0][bhp_id]"
                                        class="bhp-select w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm focus:border-sky-500 focus:outline-none focus:ring-4 focus:ring-sky-100">
                                        <option value="">Pilih Bahan Habis Pakai</option>
                                        @foreach ($dataBahanHabisPakai as $bhp)
                                            <option value="{{ $bhp->id }}">{{ $bhp->nama_barang }}</option>
                                        @endforeach
                                    </select>
                                </div>

                                <div class="xl:col-span-2">
                                    <label class="mb-1.5 block text-sm font-medium text-slate-700">Batch Item</label>
                                    <select
                                        class="batch-bhp-select w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm focus:border-sky-500 focus:outline-none focus:ring-4 focus:ring-sky-100">
                                        <option value="">Pilih / ketik batch baru</option>
                                    </select>

                                    <input type="hidden" name="details[0][batch_bhp_id]" class="batch-bhp-id-input"
                                        value="">
                                    <input type="hidden" name="details[0][batch_nama]" class="batch-bhp-nama-input"
                                        value="">

                                    <p class="mt-1 text-xs text-slate-500">
                                        Pilih batch yang sudah ada jika sama. Jika belum ada, ketik nama batch baru lalu
                                        tekan enter.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-4 xl:grid-cols-3 my-2.5">
                                <div class="xl:col-span-1">
                                    <label class="mb-1.5 block text-sm font-medium text-slate-700">Tanggal
                                        Kadaluarsa</label>
                                    <input type="date" name="details[0][tanggal_kadaluarsa_bahan_habis_pakai]"
                ...
                <div
                    ...
                    <div class="flex flex-col-reverse gap-2 sm:flex-row">
                        <button type="button" id="button-cancel-modal-create-restock-bhp"
...
<template id="template-detail-row-restock-bhp">
    <div class="detail-row rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-4">
            <div class="xl:col-span-2">
                <label class="mb-1.5 block text-sm font-medium text-slate-700">Bahan Habis Pakai</label>
                <select name="details[__INDEX__][bhp_id]"
                    class="bhp-select w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm focus:border-sky-500 focus:outline-none focus:ring-4 focus:ring-sky-100">
                    <option value="">Pilih Bahan Habis Pakai</option>
                    @foreach ($dataBahanHabisPakai as $bhp)
                        <option value="{{ $bhp->id }}">{{ $bhp->nama_barang }}</option>
            ...
            <div class="xl:col-span-2">
                ...
                <input type="hidden" name="details[__INDEX__][batch_bhp_id]" class="batch-bhp-id-input"
                    value="">
                <input type="hidden" name="details[__INDEX__][batch_nama]" class="batch-bhp-nama-input"
                    value="">

                <p class="mt-1 text-xs text-slate-500">
                    Pilih batch yang sudah ada jika sama. Jika belum ada, ketik nama batch baru lalu tekan enter.
                </p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-3 my-2.5">
            <div class="xl:col-span-1">
                ...
                <input type="date" name="details[__INDEX__][tanggal_kadaluarsa_bahan_habis_pakai]"
                    class="tanggal-kadaluarsa-input w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm focus:border-sky-500 focus:outline-none focus:ring-4 focus:ring-sky-100">
            </div>
        ...
        <div class="flex items-start justify-between border-b border-slate-200 px-6 py-5">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Lengkapi data supplier baru untuk kebutuhan restock Bahan Habis Pakai.
                    </p>
                </div>
            </div>
...
<div id="modal-detail-restock-bhp"
    ...
    <div class="w-full max-w-6xl max-h-[95vh] overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        <div class="flex items-start justify-between border-b border-slate-200 px-6 py-5">
            <div class="flex items-start gap-4">
                ...
                <div>
                    <h3 class="text-xl font-bold text-slate-800">
                        Detail Restock Bahan Habis Pakai
                    </h3>
                    <p class="mt-1 text-sm text-slate-500">
                        Informasi lengkap transaksi restock Bahan Habis Pakai.
                    </p>
                </div>
            </div>

            <button type="button" id="button-close-modal-detail-restock-bhp"
                class="rounded-xl p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                <i class="fa-solid fa-xmark text-lg"></i>
            </button>
        </div>
                    ...
                    <table class="min-w-full text-sm border border-slate-200 rounded-xl overflow-hidden">
                        <thead class="bg-slate-100 text-slate-700">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left">Bahan Habis Pakai</th>
                        ...
                        <tbody id="detail-restock-bhp-tbody" class="divide-y divide-slate-200">
                            <tr>
        ...
        <div class="border-t border-slate-200 px-6 py-4 flex justify-end">
            <button type="button" id="button-close-footer-modal-detail-restock-bhp"
                class="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                Tutup
            </button>
        </div>
...
@vite(['resources/js/farmasi/restock-bahan-habis-pakai/data-restock-bahan-habis-pakai.js'])
