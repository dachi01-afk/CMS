...
<body class="bg-slate-50">
    ...
    <script>
        $(document).ready(function() {
            ...
            const defaultPeriode = @json($defaultPeriode);
            const defaultTanggal = @json($defaultTanggal);
            const defaultMinggu = @json($defaultMinggu);
            const defaultBulan = @json($defaultBulan);
            const defaultTahun = @json($defaultTahun);
            ...
            function loadPenjualanData() {
                ...
                $.getJSON("{{ route('farmasi.penjualan-obat.data') }}", getFilterParams())
            ...
            function resetFilter() {
                $("#filterPeriodePenjualan").val(defaultPeriode);
                $("#filterTanggalPenjualan").val(defaultTanggal);
                $("#filterMingguPenjualan").val(defaultMinggu);
                $("#filterBulanPenjualan").val(defaultBulan);
                $("#filterTahunPenjualan").val(defaultTahun);

                toggleFilterInputs();
                loadPenjualanData();
            }

            $("#filterPeriodePenjualan").on("change", function() {
                toggleFilterInputs();
            });

            $("#btnApplyPenjualanFilter").on("click", function() {
                loadPenjualanData();
            });

            $("#btnResetPenjualanFilter").on("click", function() {
                resetFilter();
            });
