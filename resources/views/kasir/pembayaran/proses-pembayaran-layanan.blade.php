        ...
        <div class="mx-auto max-w-3xl">
            ...
            <div class="mt-6 space-y-4 border-y border-gray-200 py-8 dark:border-gray-700 sm:mt-8">
                <h4 class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ $transaksiLayanan->pasien->nama_pasien ?? '-' }}
                </h4>
                ...
                <dl class="space-y-2">
                    <div>
                        ...
                        <dd class="mt-1 text-base font-normal text-gray-500 dark:text-gray-400">
                            @if (optional($transaksiLayanan->kunjungan)->tanggal_kunjungan)
                                {{ \Carbon\Carbon::parse($transaksiLayanan->kunjungan->tanggal_kunjungan)
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-base font-normal text-gray-500 dark:text-gray-400">
                            {{ $transaksiLayanan->kode_transaksi ?? '-' }}
                        </dd>
                    </div>
                </dl>
            </div>
                ...
                <div class="relative overflow-x-auto border-b border-gray-200 dark:border-gray-800">
                    <table class="w-full text-left font-medium text-gray-900 dark:text-white md:table-fixed">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            @if ($transaksiLayanan->layanan)
                                <tr>
                                    <td class="whitespace-nowrap py-4 md:w-[320px] px-4">
                                        {{ $transaksiLayanan->layanan->nama_layanan }}
                                        @if($transaksiLayanan->layanan->kategoriLayanan)
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                Kategori: {{ $transaksiLayanan->layanan->kategoriLayanan->nama_kategori }}
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4 text-center text-base font-normal">
                                        x{{ $transaksiLayanan->jumlah ?? 1 }}
                                    </td>
                                    <td class="px-4 py-4 text-right text-base font-bold">
                                        Rp{{ number_format($transaksiLayanan->sub_total ?? $transaksiLayanan->total_tagihan ?? 0, 0, ',', '.') }}
                                    </td>
                                </tr>
                            @else
                                <tr>
                    ...
                    <div class="space-y-4">
                        ...
                        <dl class="flex items-center justify-between gap-4">
                            ...
                            <dd class="text-base font-medium text-gray-900 dark:text-white">
                                Rp{{ number_format($transaksiLayanan->total_tagihan ?? 0, 0, ',', '.') }}
                            </dd>
                        </dl>
                        ...
                        <input type="hidden" id="total_setelah_diskon"
                               value="{{ $transaksiLayanan->total_tagihan ?? 0 }}">
                    </div>
...
{{-- total awal (hidden) --}}
<input type="hidden" id="total_tagihan_awal" value="{{ $transaksiLayanan->total_tagihan ?? 0 }}">
...
<div id="pembayaranCash" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-md p-4">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <form id="formPembayaranCash"
                  action="{{ route('kasir.layanan.pembayaran.cash') }}"
                  method="POST">
                @csrf
                <div class="p-4 space-y-4">
                    <input type="hidden" name="id" value="{{ $transaksiLayanan->id }}">
                    ...
                    <div>
                        ...
                        <div class="relative mt-1">
                            ...
                            <input type="text" id="total_tagihan_cash" readonly
                                   value="{{ number_format($transaksiLayanan->total_tagihan ?? 0, 0, ',', '.') }}"
...
<div id="pembayaranTransfer" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-2xl p-4">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <form id="formPembayaranTransfer"
                  action="{{ route('kasir.layanan.pembayaran.transfer') }}"
                  method="POST" enctype="multipart/form-data">
                @csrf
                <div class="p-4 space-y-4">
                    <input type="hidden" name="id" value="{{ $transaksiLayanan->id }}">
                    <input type="hidden" name="metode_pembayaran_id" id="metode-pembayaran-transfer" value="">
            ...
            formPembayaranTransfer.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    const data = ct.includes('application/json')
                        ? await response.json()
                        : { success: response.ok, message: response.ok ? 'OK' : 'Gagal' };
