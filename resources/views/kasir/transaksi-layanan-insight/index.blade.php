...
<head>
    ...
    <title>CMS-Royal-Klinik | Insight Transaksi Layanan</title>
    ...
    <style>
        body {
            min-height: 100vh;
            background:
                radial-gradient(circle at top left, rgba(245, 158, 11, 0.10), transparent 20%),
                radial-gradient(circle at top right, rgba(251, 191, 36, 0.08), transparent 18%),
                linear-gradient(to bottom, #fffdf7, #f8fafc);
        }
    </style>
</head>
...
<body class="text-slate-900">
    <main class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="space-y-6">
            ...
            <section
                class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-amber-900 to-amber-700 px-6 py-7 shadow-xl">
                <div class="absolute inset-0 opacity-10">
                    <div class="absolute -top-10 -right-10 h-40 w-40 rounded-full bg-white blur-2xl"></div>
                    <div class="absolute bottom-0 left-0 h-32 w-32 rounded-full bg-amber-300 blur-2xl"></div>
                </div>

                <div class="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div class="max-w-2xl">
                        <div
                            class="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-amber-100">
                            <i class="fa-solid fa-stethoscope"></i>
                            Detail Transaksi Layanan
                        </div>

                        <h1 class="mt-4 text-3xl font-black tracking-tight text-white md:text-4xl">
                            Insight Transaksi Layanan
                        </h1>

                        <p class="mt-2 text-sm leading-6 text-amber-100 md:text-base">
                            Monitoring transaksi layanan non kategori pemeriksaan.
                        </p>
                    </div>
            ...
            <section id="chartSection" data-chart-url="{{ route('kasir.insight.layanan.chart') }}"
                class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">Grafik Transaksi Layanan</h2>
                        <p class="mt-1 text-sm text-slate-500">
                            Jumlah transaksi dan pendapatan layanan berdasarkan periode.
                        </p>
                    </div>

                    <select id="filterChart"
                        class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100">
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900">Daftar Transaksi Layanan</h2>
                        <p class="mt-1 text-sm text-slate-500">
                            Cari transaksi berdasarkan kode atau status transaksi.
                        </p>
                    </div>
                    ...
                    <form method="GET" class="flex w-full max-w-md items-center gap-2">
                        <div class="relative flex-1">
                            <input type="text" name="search" value="{{ $search }}"
                                ...
                                class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 pr-10 text-sm text-slate-700 shadow-sm outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100">
                            <i
                            ...
                            @forelse ($rows as $row)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="px-4 py-3 text-sm text-slate-600">
                                        {{ \Carbon\Carbon::parse($row->tanggal_order)->format('d-m-Y H:i') }}
                                    </td>
                                    <td class="px-4 py-3 text-sm">
                                        <span
                                            class="rounded-full px-3 py-1 text-xs font-bold {{ $row->status_order_layanan === 'Sudah Bayar' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
                                            {{ $row->status_order_layanan }}
                                        </span>
                                    </td>
