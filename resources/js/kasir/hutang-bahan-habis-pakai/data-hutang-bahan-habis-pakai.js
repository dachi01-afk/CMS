...
$(function () {
    const table = $("#table-hutang-bhp").DataTable({
        processing: true,
        serverSide: true,
        paging: true,
        searching: true,
        ...
        lengthChange: false,
        info: false,
        ajax: "/kasir/hutang-bahan-habis-pakai/get-data-hutang-bahan-habis-pakai",
        columns: [
            {
                data: "DT_RowIndex",
                name: "DT_RowIndex",
...
$(function () {
    ...
    function fillDetailHutang(data) {
        ...
        const restockBhp = data.restock_bahan_habis_pakai ?? {};
        const depot = restockBhp.depot ?? {};
        const detailItems = Array.isArray(
            restockBhp.restock_bahan_habis_pakai_detail,
        )
            ? restockBhp.restock_bahan_habis_pakai_detail
            : [];
        const dibuatOleh = data.dibuat_oleh ?? null;
        const diupdateOleh = data.diupdate_oleh ?? null;
        ...
        setText("detail-restock-no-faktur", restockBhp.no_faktur ?? "-");
        setText(
            "detail-restock-tanggal-terima",
            formatTanggal(restockBhp.tanggal_terima),
        );
        setText(
            "detail-restock-jatuh-tempo",
            formatTanggal(restockBhp.tanggal_jatuh_tempo),
        );
        setText("detail-restock-status", restockBhp.status_restock ?? "-");
        setText("detail-restock-depot", depot.nama_depot ?? "-");
        setText(
            "detail-restock-total-tagihan",
            formatRupiah(restockBhp.total_tagihan),
        );
        ...
        detailItems.forEach((item, index) => {
            ...
            <td class="px-4 py-3">${item.bahan_habis_pakai?.kode ?? "-"}</td>
            <td class="px-4 py-3">${item.bahan_habis_pakai?.nama_barang ?? "-"}</td>
            ...
            <td class="px-4 py-3 text-right">${
                item.diskon_type === "persen"
                    ? `${Number(item.diskon_value ?? 0)}%`
                    : formatRupiah(item.diskon_value)
            }</td>
    ...
    document.addEventListener("click", async function (e) {
        ...
        try {
            const response = await axios.get(
                `/kasir/hutang-bahan-habis-pakai/get-data-detail-hutang-bahan-habis-pakai/${encodeURIComponent(noFaktur)}`,
            );
            hideAlert();
    ...
    document.addEventListener("click", function (e) {
        ...
        window.location.href = `/kasir/hutang-bahan-habis-pakai/pembayaran/${encodeURIComponent(noFaktur)}`;
    });
});
