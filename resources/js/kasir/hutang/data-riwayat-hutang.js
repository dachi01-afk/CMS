...
$(function () {
    window.tableRiwayatHutang = $("#table-data-riwayat-hutang").DataTable({
        processing: true,
        serverSide: true,
        paging: true,
        searching: true,
        ...
        lengthChange: false,
        info: false,
        ajax: "/kasir/riwayat-hutang/get-data-riwayat-hutang",
        columns: [
            {
                data: "DT_RowIndex",
                name: "DT_RowIndex",
    ...
    $("#data-riwayat-hutang-search-input").on("keyup", function () {
        window.tableRiwayatHutang.search(this.value).draw();
    });
    ...
    function updatePagination() {
        const info = window.tableRiwayatHutang.page.info();
    ...
    $pagination.on("click", "a", function (e) {
        ...
        if ($link.attr("id") === "btnPrev") {
            window.tableRiwayatHutang.page("previous").draw("page");
        } else if ($link.attr("id") === "btnNext") {
            window.tableRiwayatHutang.page("next").draw("page");
        } else if ($link.hasClass("page-number")) {
            window.tableRiwayatHutang
    ...
    $perPage.on("change", function () {
        window.tableRiwayatHutang.page.len(parseInt($(this).val(), 10)).draw();
    });

    window.tableRiwayatHutang.on("draw", updatePagination);
    updatePagination();
});

$(function () {
    const $modal = $("#modal-detail-riwayat-hutang");
    const $loading = $("#detail-riwayat-hutang-loading");
    const $content = $("#detail-riwayat-hutang-content");
    const $error = $("#detail-riwayat-hutang-error");

    function openModalDetailRiwayatHutang() {
        $modal.removeClass("hidden").addClass("flex");
        $("body").addClass("overflow-hidden");
    }

    function closeModalDetailRiwayatHutang() {
        $modal.addClass("hidden").removeClass("flex");
        $("body").removeClass("overflow-hidden");
    }

    function resetModalDetailRiwayatHutang() {
        $loading.addClass("hidden");
        $content.addClass("hidden");
        $error.addClass("hidden").text("");

        $("#detail-riwayat-hutang-no-faktur").text("-");
        $("#detail-riwayat-hutang-status-hutang").html("-");
        $("#detail-riwayat-hutang-tanggal-hutang").text("-");
        $("#detail-riwayat-hutang-tanggal-jatuh-tempo").text("-");
        $("#detail-riwayat-hutang-tanggal-pelunasan").text("-");
        $("#detail-riwayat-hutang-metode-pembayaran").text("-");
        $("#detail-riwayat-hutang-total-hutang").text("Rp 0");

        $("#detail-riwayat-hutang-nama-supplier").text("-");
        $("#detail-riwayat-hutang-kontak-person").text("-");
        $("#detail-riwayat-hutang-no-hp-supplier").text("-");
        $("#detail-riwayat-hutang-email-supplier").text("-");
        $("#detail-riwayat-hutang-alamat-supplier").text("-");

        $("#detail-riwayat-hutang-no-faktur-restock").text("-");
        $("#detail-riwayat-hutang-tanggal-terima").text("-");
        $("#detail-riwayat-hutang-tanggal-jatuh-tempo-restock").text("-");
        $("#detail-riwayat-hutang-status-restock").html("-");
        $("#detail-riwayat-hutang-depot").text("-");
        $("#detail-riwayat-hutang-total-tagihan-restock").text("Rp 0");

        $("#detail-riwayat-hutang-dibuat-oleh").text("-");
        $("#detail-riwayat-hutang-diupdate-oleh").text("-");
        $("#detail-riwayat-hutang-created-at").text("-");
        $("#detail-riwayat-hutang-updated-at").text("-");

        $("#detail-riwayat-hutang-detail-item-body").html(`
            <tr>
                <td colspan="10" class="px-4 py-4 text-center text-slate-500">Belum ada data item.</td>
            </tr>
        `);

        $("#detail-riwayat-hutang-bukti-pembayaran-wrapper").html(`
            <p class="text-sm text-slate-500">-</p>
        `);
    }

    function showLoadingModal() {
        resetModalDetailRiwayatHutang();
        $loading.removeClass("hidden");
        openModalDetailRiwayatHutang();
    }
    ...
    function setDetailContent(data) {
        $("#detail-riwayat-hutang-no-faktur").text(data.no_faktur ?? "-");
        $("#detail-riwayat-hutang-status-hutang").html(
        ...
        $("#detail-riwayat-hutang-total-hutang").text(
        ...
        $("#detail-riwayat-hutang-depot").text(data.restock?.depot ?? "-");
        $("#detail-riwayat-hutang-total-tagihan-restock").text(
    ...
    function renderDetailItems(items) {
        ...
        const rows = items
            .map(
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <td class="px-4 py-3">${item.kode_obat ?? "-"}</td>
        ...
        $("#detail-riwayat-hutang-detail-item-body").html(rows);
    }
    ...
    $(document).on("click", ".button-detail-riwayat-hutang", async function () {
        ...
        try {
            const response = await axios.get(
                `/kasir/riwayat-hutang/get-data-detail-riwayat-hutang/${encodeURIComponent(noFaktur)}`,
            );
    ...
    $("#close-modal-detail-riwayat-hutang").on("click", function () {
        closeModalDetailRiwayatHutang();
    });
