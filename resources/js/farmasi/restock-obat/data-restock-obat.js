...
$(function () {
    window.tableRestockObat = $("#table-restock-obat").DataTable({
        processing: true,
        serverSide: true,
        paging: true,
        searching: true,
        ...
        lengthChange: false,
        info: false,
        ajax: "/farmasi/restock-obat/get-data-restock-obat",
        columns: [
            {
                data: "DT_RowIndex",
                name: "DT_RowIndex",
    ...
    $("#restock-obat-search-input").on("keyup", function () {
        window.tableRestockObat.search(this.value).draw();
    });

    const $info = $("#restock-obat-custom-info");
    const $pagination = $("#restock-obat-custom-pagination");
    const $perPage = $("#restock-obat-page-length");

    function updatePagination() {
        const info = window.tableRestockObat.page.info();
    ...
    $pagination.on("click", "a", function (e) {
        ...
        if ($link.attr("id") === "btnPrev") {
            window.tableRestockObat.page("previous").draw("page");
        } else if ($link.attr("id") === "btnNext") {
            window.tableRestockObat.page("next").draw("page");
        } else if ($link.hasClass("page-number")) {
            window.tableRestockObat
    ...
    $perPage.on("change", function () {
        window.tableRestockObat.page.len(parseInt($(this).val(), 10)).draw();
    });

    window.tableRestockObat.on("draw", updatePagination);
    updatePagination();
});
...
$(function () {
    ...
    const pageEl = document.getElementById("restock-obat-page");
    ...
    const createRestockObatUrl = pageEl
        ? pageEl.dataset.createRestockObatUrl
        : "";
    ...
    $("#button-open-modal-create-restock-obat").on("click", function () {
        openModalRestock();
        calculateAllRows();
    });

    $("#button-close-modal-create-restock-obat").on("click", function () {
        closeModalRestock();
    });

    $("#button-cancel-modal-create-restock-obat").on("click", function () {
        closeModalRestock();
    });
    ...
    function buildBatchUrl(obatId) {
        return batchUrlTemplate.replace(":obatId", obatId);
    }
    ...
    $("#button-add-detail-row").on("click", function () {
        ...
        const template = document.getElementById(
            "template-detail-row-restock-obat",
        );
    ...
    $("#form-create-restock-obat").on("submit", function (e) {
        e.preventDefault();
...
$(function () {
    ...
    $(
        "#button-close-modal-detail-restock-obat, #button-close-footer-modal-detail-restock-obat",
    ).on("click", function () {
    ...
    function openModalDetailRestockObat() {
        lockBodyScroll();
        $("#modal-detail-restock-obat").removeClass("hidden").addClass("flex");
    }

    function closeModalDetailRestockObat() {
        unlockBodyScroll();
        $("#modal-detail-restock-obat").removeClass("flex").addClass("hidden");
    }
    ...
    function resetModalDetailRestockObat() {
        ...
        $("#detail-restock-obat-tbody").html(`
            <tr>
    ...
    function fillModalDetailRestockObat(data) {
        ...
        $("#detail-restock-obat-tbody").html(html);
    }

    $(document).on("click", ".button-detail-restock-obat", function () {
...
$(function () {
    $(document).on("click", ".button-cancel-restock-obat", function () {
        const id = $(this).data("id");
        const noFaktur = $(this).data("noFaktur");

        Swal.fire({
            title: "Batalkan restock obat?",
            html: `
            ...
            $.ajax({
                url: `/farmasi/restock-obat/cancel/${noFaktur}`,
                type: "POST",
                data: {
