...
$(function () {
    window.tableStokMasukkObat = $("#table-stok-masuk-obat").DataTable({
        processing: true,
        serverSide: true,
        paging: true,
        searching: true,
        ...
        lengthChange: false,
        info: false,
        ajax: "/farmasi/stok-masuk-obat/get-data-stok-masuk-obat",
        columns: [
            {
                data: "DT_RowIndex",
                name: "DT_RowIndex",
    ...
    $("#stok-masuk-obat-search-input").on("keyup", function () {
        window.tableStokMasukkObat.search(this.value).draw();
    });
    ...
    function updatePagination() {
        const info = window.tableStokMasukkObat.page.info();
    ...
    $pagination.on("click", "a", function (e) {
        ...
        if ($link.attr("id") === "btnPrev") {
            window.tableStokMasukkObat.page("previous").draw("page");
        } else if ($link.attr("id") === "btnNext") {
            window.tableStokMasukkObat.page("next").draw("page");
        } else if ($link.hasClass("page-number")) {
            window.tableStokMasukkObat
    ...
    $perPage.on("change", function () {
        window.tableStokMasukkObat.page.len(parseInt($(this).val(), 10)).draw();
    });

    window.tableStokMasukkObat.on("draw", updatePagination);
    updatePagination();
...
$(function () {
    ...
    function fillModalDetailStokMasukObat(data) {
        ...
        $("#detail-total-tagihan").text(formatRupiah(data.total_tagihan));
        ...
        details.forEach((detail, index) => {
            html += `
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-4 py-3">${detail.kode_obat || "-"}</td>
                    <td class="px-4 py-3">${detail.nama_obat || "-"}</td>
                    <td class="px-4 py-3">${detail.nama_batch || "-"}</td>
                    <td class="px-4 py-3">${formatDateIndonesia(detail.tanggal_kadaluarsa_obat) || "-"}</td>
    ...
    $(document).on("click", ".btn-detail-stok-masuk", function () {
        ...
        $.ajax({
            url: `/farmasi/stok-masuk-obat/get-data-detail-stok-masuk-obat/${noFaktur}`,
            type: "GET",
    ...
    $(
        "#btn-close-modal-detail-stok-masuk-obat, #btn-close-footer-modal-detail-stok-masuk-obat",
    ).on("click", function () {
        closeModalDetail();
    });
    ...
    $(document).on("click", ".btn-konfirmasi-stok-masuk", function () {
        ...
        Swal.fire({
            ...
            text: "Stok obat akan ditambahkan ke stok global, stok depot, dan stok batch depot.",
            icon: "warning",
            ...
            $.ajax({
                url: `/farmasi/stok-masuk-obat/konfirmasi/${id}`,
                type: "POST",
                data: {
                ...
                beforeSend: function () {
                    Swal.fire({
                        title: "Memproses...",
                        text: "Sedang mengonfirmasi stok masuk obat.",
                ...
                success: function (response) {
                    if (window.tableStokMasukkObat) {
                        window.tableStokMasukkObat.ajax.reload(null, false);
                    }

                    if (window.tableRiwayatStokMasukObat) {
                        window.tableRiwayatStokMasukObat.ajax.reload(
                            null,
                            false,
                        );
                    }
