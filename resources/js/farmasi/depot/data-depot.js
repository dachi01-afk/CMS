            ...
            tableRepair = $("#table-repair-obat").DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        className: "border border-gray-300 px-3 py-2 text-center col-qty-akhir",
                        render: (data) => data || 0,
                    },
                    {
                        data: null,
                    ...
                    // ... di dalam konfigurasi columns DataTable ...
{
    data: null,
    render: (data, type, row) => {
        let displaySelisih = "0";
        let colorClass = "";
        // Samakan cara ambil ID dengan kolom input agar sinkron
        const idObat = row.obat_id ? row.obat_id : row.id;

        if (repairDataState[idObat] !== undefined) {
            const qtySistem = parseFloat(row.pivot.stok_obat || 0);
            const qtyFisik = parseFloat(repairDataState[idObat]);
            
            const diff = qtyFisik - qtySistem;
            
            if (!isNaN(diff)) {
                // Tampilkan + jika Fisik > Sistem
                displaySelisih = diff > 0 ? `+${diff}` : diff.toString();
                colorClass = diff === 0 ? "text-green-500" : "text-red-500";
            }
        }
...
},
{
    ...
    $(document).on("input", ".input-qty-fisik", function () {
        ...
        const qtySistem = parseFloat($row.find(".col-qty-akhir").text()) || 0;
        ...
        if (qtyFisikVal === "" || isNaN(qtyFisik)) {
            ...
            $ketInput.val("Belum Diisi").removeClass("text-red-600 text-green-600");
            delete repairDataState[obatId];
            return;
        }

        repairDataState[obatId] = qtyFisikVal;

        // LOGIKA REVISI: Fisik - Sistem
        const selisih = qtyFisik - qtySistem;
        
        // Tampilkan lambang + jika positif
        const teksSelisih = selisih > 0 ? `+${selisih}` : selisih;
        $selisihCell.text(teksSelisih);

        if (selisih === 0) {
            $ketInput.val("Sesuai").addClass("text-green-600").removeClass("text-red-600");
            $selisihCell.addClass("text-green-500").removeClass("text-red-500");
        } else {
            $ketInput.val("Tidak Sesuai").addClass("text-red-600").removeClass("text-green-600");
            $selisihCell.addClass("text-red-500").removeClass("text-green-500");
        }
    });

    // --- AJAX SUBMIT ---
    $(document).on("click", "#btn-save-repair-obat", function (e) {
        e.preventDefault();
        const $btn = $(this);
        ...
        if (items.length === 0) {
            alert("Silahkan isi minimal satu data fisik obat sebelum menyimpan.");
            return;
        }
    ...
    function updatePagination() {
        ...
        $info.text(`Menampilkan ${startNode}–${endNode} dari ${totalRecords} data`);
        $pagination.empty();

        const prevDisabled = currentPage === 1 ? "opacity-50 pointer-events-none" : "";
        $pagination.append(`<li><a href="#" id="btnPrev" class="flex items-center justify-center px-3 h-8 text-gray-500 bg-white border border-gray-300 rounded-s-lg hover:bg-gray-100 ${prevDisabled}">Prev</a></li>`);

        for (let i = 1; i <= totalPages; i++) {
            const active = i === currentPage ? "text-blue-600 bg-blue-50 border-blue-300 z-10" : "text-gray-500 bg-white border-gray-300 hover:bg-gray-100";
            $pagination.append(`<li><a href="#" class="page-number flex items-center justify-center px-4 h-8 border ${active}" data-page="${i}">${i}</a></li>`);
        }

        const nextDisabled = currentPage === totalPages || totalPages === 0 ? "opacity-50 pointer-events-none" : "";
        $pagination.append(`<li><a href="#" id="btnNext" class="flex items-center justify-center px-3 h-8 text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 ${nextDisabled}">Next</a></li>`);
    }
...
$(function () {
    const elemetModalRepairBHP = document.getElementById("modal-repair-bhp");
    const modalRepairBHP = elemetModalRepairBHP
        ? new Modal(elemetModalRepairBHP, {
              backdrop: "static",
              closable: false,
          })
        : null;

    const $formRepairBHP = $("#form-modal-repair-bhp");
    let tableRepair = null;
    ...
    window.resetModalRepairBHP = function () {
    ...
    window.closeModalRepairBHP = function () {
        if (modalRepairBHP) {
            modalRepairBHP.hide();
            resetModalRepairBHP();
        }
    ...
    $(document).on("click", "#btn-repair-bhp", function () {
        const url = $(this).data("url");
        ...
        if ($.fn.DataTable.isDataTable("#table-repair-bhp")) {
            tableRepair.ajax.url(url).load();
        } else {
            tableRepair = $("#table-repair-bhp").DataTable({
                processing: true,
                responsive: true,
                serverSide: true,
                paging: true,
                ...
                columns: [
                    ...
                    {
                        data: "pivot.stok_barang",
                        name: "pivot.stok_barang",
                        searchable: false,
                        className:
                    ...
                    },
                    {
                        data: null,
                        render: (data, type, row) => {
                            // PASTIKAN MENGGUNAKAN row.bahan_habis_pakai_id atau row.id sesuai aslinya
                            // Jika row.id adalah ID Obat, maka ini sudah benar.
                            // Tapi jika tidak, gunakan row.bahan_habis_pakai_id
                            const idBHP = row.bahan_habis_pakai_id ? row.bahan_habis_pakai_id : row.id;
                            const savedVal = repairDataState[idBHP] || "";
                            return `
                <div class="px-2">
                    <input type="number" name="qty_fisik[]" data-id="${idBHP}" 
                        class="input-qty-fisik w-full border-b border-gray-400 focus:border-blue-600 outline-none text-center bg-transparent py-1 transition-all" 
                        placeholder="0" value="${savedVal}">
                </div>`;
                        },
    ...
    $(document).on("input", ".input-qty-fisik", function () {
        ...
        if (qtyFisikVal === "" || isNaN(qtyFisik)) {
            ...
            $ketInput
                .val("Belum Diisi")
                .removeClass("text-red-600 text-green-600");
            delete repairDataState[bhpId]; // Hapus dari state jika kosong
            return;
        }

        // Simpan ke State
        repairDataState[bhpId] = qtyFisikVal;
    ...
    // --- AJAX SUBMIT ---
    $(document).on("click", "#btn-save-repair-bhp", function (e) {
        e.preventDefault();
        const $btn = $(this);
        const url = $formRepairBHP.data("url");

        // Validasi state tidak boleh kosong
        const items = Object.keys(repairDataState).map((id) => ({
            bahan_habis_pakai_id: id,
            qty_fisik: repairDataState[id],
        }));
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if ($.fn.DataTable.isDataTable("#table-repair-bhp")) {
                    $("#table-repair-bhp").DataTable().ajax.reload(null, false);
                }
            },
            error: function (xhr) {
                const msg =
    ...
    // --- GLOBAL SEARCH OBAT ---
    const $globalSearchBHP = $("#globalSearchBHP");
    let searchTimer = null;
    let lastValue = "";
    let inflightXhr = null;

    if ($globalSearchBHP.length) {
        const runSearch = (value) => {
            if (value === lastValue) return;
            lastValue = value;
            tableRepair.search(value).draw();
        ...
        $globalSearchBHP.on("input", function () {
            const value = $(this).val().trim();
        ...
        $globalSearchBHP.on("keydown", function (e) {
            if (e.key === "Enter") {
                e.preventDefault();
                clearTimeout(searchTimer);
                runSearch($(this).val().trim());
