$(function () {
    var table = $('#transaksiTable').DataTable({
        ...
        lengthChange: false,
        info: false,
        ajax: "/laporan/laporan_transaksi_apoteker",
        columns: [
            { data: 'id', name: 'id' },
            { data: 'kode_resep', name: 'kode_resep' },
            { data: 'nama_apoteker', name: 'nama_apoteker' },
            { data: 'tanggal_transaksi_apoteker', name: 'tanggal_transaksi_apoteker' },
    ...
    $('#transaksi_searchInput').on('keyup', function () {
        table.search(this.value).draw();
    });

    const $info = $('#transaksi_customInfo');
    const $pagination = $('#transaksi_customPagination');
    const $perPage = $('#transaksi_pageLength');
